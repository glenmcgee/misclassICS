...
scenario <-  "GN25_BothDec"     #"G0_Fixed" "G0_BothDec" "G0_BothInc" "G0_SensDec"
...
gendat <- function(nu_00=logit(0.90), ## baseline specificities
  ...
  mdat <- genICS(gamma_bk=gam,gamma1_bk=gam,min_Nk=m,slopes=TRUE,cond=TRUE,  K=KK,Xrate=Xprev,alpha0=a0,alpha1=a1,beta0=b0,beta1=b1,beta2=b2,sig.bk=sigma0,sig1.bk=sigma1) ## marginal
...
fit_complete <- function(datMIS,valid=FALSE,startpos=FALSE){
  ...
  ## GLMM
  mm <- try(JMMICS_fit(Nk=df2$Nk,Zk=cbind(1,df2$X1k),Yki=df$Yki,Xki=cbind(1,df$X1ki,df$X2ki),IDk=df2$IDk,Z0k=NULL,minNk=1,NegBin=FALSE,ZIP=FALSE,slopes=TRUE,slope_col=2,condSize=TRUE,condOut=TRUE,joint=FALSE,startpos=startpos,nquad=60),silent=T)
  if(length(mm)>1){
  ...
  ## JM
  jmm <- try(JMMICS_fit(Nk=df2$Nk,Zk=cbind(1,df2$X1k),Yki=df$Yki,Xki=cbind(1,df$X1ki,df$X2ki),IDk=df2$IDk,Z0k=NULL,minNk=1,NegBin=FALSE,ZIP=FALSE,slopes=TRUE,slope_col=2,condSize=TRUE,condOut=TRUE,joint=TRUE,startpos=startpos,nquad=60),silent=T)
  if(length(jmm)>1){
...
fit_mis <- function(datMIS,startpos=FALSE){
  ...
  ## GLMM
  mm <- try(JMMICS_fit(Nk=df2$Nk,Zk=cbind(1,df2$W1k),Yki=df$Yki,Xki=cbind(1,df$W1ki,df$X2ki),IDk=df2$IDk,Z0k=NULL,minNk=1,NegBin=FALSE,ZIP=FALSE,slopes=TRUE,slope_col=2,condSize=TRUE,condOut=TRUE,joint=FALSE,startpos=startpos,nquad=60),silent=T)
  if(length(mm)>1){
    est_mm <- mm$ests; SE_mm <- mm$SEs
  }else{est_mm <- SE_mm <- rep(NA,nb+ns)}
  if(length(SE_mm)<length(est_mm)){SE_mm <- rep(NA,length(est_mm))} ## error handling
  ...
  ## JM
  jmm <- try(JMMICS_fit(Nk=df2$Nk,Zk=cbind(1,df2$W1k),Yki=df$Yki,Xki=cbind(1,df$W1ki,df$X2ki),IDk=df2$IDk,Z0k=NULL,minNk=1,NegBin=FALSE,ZIP=FALSE,slopes=TRUE,slope_col=2,condSize=TRUE,condOut=TRUE,joint=TRUE,startpos=startpos,nquad=60),silent=T)
  if(length(jmm)>1){
    est_jmm <- jmm$ests; SE_jmm <- jmm$SEs
  }else{est_jmm <- SE_jmm <- rep(NA,na+ng+nb+ns)}
  if(length(SE_jmm)<length(est_jmm)){SE_jmm <- rep(NA,length(est_jmm))} ## error handling
...
fit_obsLik <- function(datMIS,dependN=TRUE,startpos=FALSE){
  ...
  if(dependN==FALSE){ ## misclassification model does not include N
    ...
    ## JM
    jmm <- try(MISclassICS_fit(Nk=df2$Nk,Zk=cbind(1,df2$X1k),Yki=df$Yki,Xki=cbind(1,df$X1ki,df$X2ki),Wk=df2$W1k,Zwk=cbind(1,df2$X1k),Xk=df2$X1k,Zxk=matrix(1,nrow=nrow(df2),ncol=1),VALIDk=df2$VALIDk,IDk=df2$IDk,Z0k=NULL,minNk=1,NegBin=FALSE,ZIP=FALSE,slopes=TRUE,slope_col=2,condSize=TRUE,condOut=TRUE,joint=TRUE,startpos=startpos,nquad=60),silent=T)
    if(length(jmm)>1){
    ...
    ## GLMM
    mm <- try(MISclassICS_fit(Nk=df2$Nk,Zk=cbind(1,df2$X1k),Yki=df$Yki,Xki=cbind(1,df$X1ki,df$X2ki),Wk=df2$W1k,Zwk=cbind(1,df2$X1k,df2$Nk,df2$X1k*df2$Nk),Xk=df2$X1k,Zxk=matrix(1,nrow=nrow(df2),ncol=1),VALIDk=df2$VALIDk,IDk=df2$IDk,Z0k=NULL,minNk=1,NegBin=FALSE,ZIP=FALSE,slopes=TRUE,slope_col=2,condSize=TRUE,condOut=TRUE,joint=FALSE,interac=TRUE,inter_id=4,main_id=3,startpos=startpos,nquad=60),silent=T)
    if(length(mm)>1){
      est_mm <- mm$ests[1:5]; SE_mm <- mm$SEs[1:5]
    }else{est_mm <- SE_mm <- rep(NA,nb+ns)}
    if(length(SE_mm)<length(est_mm)){SE_mm <- rep(NA,length(est_mm))} ## error handling
    ...
    ## JM
    jmm <- try(MISclassICS_fit(Nk=df2$Nk,Zk=cbind(1,df2$X1k),Yki=df$Yki,Xki=cbind(1,df$X1ki,df$X2ki),Wk=df2$W1k,Zwk=cbind(1,df2$X1k,df2$Nk,df2$X1k*df2$Nk),Xk=df2$X1k,Zxk=matrix(1,nrow=nrow(df2),ncol=1),VALIDk=df2$VALIDk,IDk=df2$IDk,Z0k=NULL,minNk=1,NegBin=FALSE,ZIP=FALSE,slopes=TRUE,slope_col=2,condSize=TRUE,condOut=TRUE,joint=TRUE,interac=TRUE,inter_id=4,main_id=3,startpos=startpos,nquad=60),silent=T)
    if(length(jmm)>1){
      est_jmm <- jmm$ests[1:9]; SE_jmm <- jmm$SEs[1:9]
    }else{est_jmm <- SE_jmm <- rep(NA,na+ng+nb+ns)}
    if(length(SE_jmm)<length(est_jmm)){SE_jmm <- rep(NA,length(est_jmm))} ## error handling
...
fitmods <- function(dat,depN=FALSE,startpos=FALSE){
  ...
  return(list(ests=c(g_complete$ests,g_mis$ests,g_valid$ests,g_obsLik$ests),
              SEs=c(g_complete$SEs,g_mis$SEs,g_valid$SEs,g_obsLik$SEs)))
...
## save
write.table(results,file=paste0(path,"results_sim",scenario,"COND",suffix,".txt"),col.names=TRUE)
